...
def load_hq_ids_from_dir(hq_dir: Path) -> Set[str]:
    """
    Extract contig IDs from all files in an HQ genome directory.
    Numeric IDs are parsed from filenames via extract_contig_id().
    """
    hq_ids = set()
    count = 0
...
def load_hq_ids_from_list(list_file: Path) -> Set[str]:
    """Load HQ contig IDs from a plain text file (one ID per line)."""
    hq_ids = set()
...
def process_assembly(assembly: Path, hq_ids: Set[str], output: Path,
    ...
    """Remove HQ contigs from assembly and optionally apply a length filter."""
    ...
    stats = {
        'total_contigs': 0, 'hq_removed': 0,
        'too_short_removed': 0, 'kept': 0,
        'total_bp': 0, 'removed_bp': 0, 'kept_bp': 0,
    }
    ...
    if stats['total_bp'] > 0:
        ...
        print(f"  Removal rate:               {100 - pct_kept:>9.2f}%")
...
Examples:
  ...
  # Remove using list file with length filter
  mamisa remove-hq-contigs \\
    --assembly assembly.fa \\
    --hq-list hq_ids.txt \\
    --output filtered.fa \\
