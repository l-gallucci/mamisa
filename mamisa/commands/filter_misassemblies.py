...
from ..utils.misassembly import load_clipping_positions
...
def get_hq_contig_ids(hq_dir: Path) -> Set[str]:
    ...
    Handles common assembler naming conventions (genome_123.fa, contig_456.fasta, etc.)
    """
    hq_ids = set()
...
def split_contig(contig_name: str, sequence: str, split_positions: List[int],
    ...
    """Split a contig at given positions and return only fragments >= min_length."""
    fragments = []
    ...
    for i in range(len(positions) - 1):
        ...
        if fragment_len >= min_length:
            fragment_name = (
                f"{contig_name}_split{i + 1}_len{fragment_len}"
...
def classify_contigs(assembly: Path, hq_ids: Set[str],
    ...
    """
    Classify all contigs in the assembly into four categories:
      - hq_clean:          HQ genomes without misassemblies
      - hq_problematic:    HQ genomes with misassemblies
      - nonhq_misassembly: Non-HQ contigs with misassemblies
      - nonhq_clean:       Non-HQ contigs without misassemblies
    """
    ...
    for contig_name, _ in read_fasta_streaming(assembly):
...
Examples:
  ...
  mamisa filter-misassemblies \\
    ...
    parser.add_argument('--preserve-hq-with-issues', action='store_true',
                        help='Keep HQ genomes with misassemblies (split instead of remove)')
